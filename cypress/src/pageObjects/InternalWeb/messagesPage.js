class MessagesPage {
	...
	static getRowByName(name) {
		return cy
			.contains(this.selectorsMessagePage.nameCell, name)
	...
	static deleteMessageByName(name) {
		this.getRowByName(name).within(() => {
			cy.get(this.selectorsMessagePage.deleteIcon).should('be.visible').click()
		})
		return this
	}
	...
	static closeMessageModal() {
		cy.get(this.selectorsMessagePage.messageModal).should('be.visible').find('button:contains("Close")').click()
